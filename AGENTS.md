...
src/mcp.ts             Holston published AS an MCP server (McpServer + createMcpHandler at /mcp; 8 tools drive the OWNER instance via getAgentByName stub)
src/lib/bearer.ts      Pure bearer gate for /mcp (extracted so tests don't pull agents/mcp's cloudflare: imports)
...
## Deferred / won't ship (v0.7.0+)

Explicit product decisions from the post-v0.7.0 assessment — do not re-open without a product need:

- **Sub-agents (`agentTool` / `subAgent`)** — deferred. No parallel task fan-out unless the product wants delegated research/work.
- **`getSkillScriptRunner`** — deferred. Skills stay instruction-only (markdown); executable helpers belong in Codemode snippets.
- **`promote_skill` action** — dropped. Pending skill approve/reject already exists via HTTP/`@callable` + Skills panel; not duplicated as a Think action.
- **`soul` context block** — dropped. Agent identity lives in the system prompt + `settings.customInstructions`.
- **Telegram in `notifyUser`** — dropped. Reminder fan-out is push + web broadcast + owner email; Telegram remains chat/messenger only.
...
- **Resilience (sentinel building blocks)**: `src/core/circuit-breaker.ts` wraps `SkillStore.search` (Workers-AI embed + Vectorize) so a brownout opens the circuit and returns the empty fallback instead of eating latency each call. `src/core/csrf.ts` `assertSameOrigin` guards the browser skills approve/reject POST endpoints (NOT the Telegram webhook — it has no Origin).
- **Notify fan-out**: `notifyUser` (used by reminders) hits push + web broadcast + owner email (when the `send_email` binding + OWNER_EMAIL are set). Telegram is an interactive messenger (`getMessengers`), not a reminder fan-out channel. `onEmail` now processes inbound mail as a blocking turn and `replyToEmail`s the model's answer (HMAC-signed when EMAIL_SIGNING_SECRET is set).
...
- **Holston AS an MCP server (SDK audit rec #1)**: `src/mcp.ts` builds a plain `McpServer` (`@modelcontextprotocol/sdk`, installed 1.29.0) with 8 tools and mounts it via `createMcpHandler(server, {route:"/mcp"})` (from `agents/mcp`) in `handleFetch` — checked BEFORE `routeAgentRequest` so `/mcp` is its own path. NOT `McpAgent` (that's a whole separate DO with no data): each tool resolves the OWNER's existing instance via `getAgentByName(env.HolstonAgent, ownerInstance(env))` and calls its public/@callable methods (listReminders/createReminder/cancelReminder/getMemory/setMemory/searchHistory/listReceipts/listEvents), so an external caller sees the SAME reminders/memory the owner sees — and writes reuse the dashboard's idempotency+receipt path. Auth is a bearer gate (`MCP_ACCESS_KEY`), independent of Cloudflare Access; **closed (404) unless the key is set**. `bearerOk` lives in `src/lib/bearer.ts` (pure, length-checked diff-accumulating compare) — extracted because importing `agents/mcp` in a vitest node env fails on its `cloudflare:` protocol imports. Tool callbacks return `{content:[{type:"text",text}]}`. `handleFetch` now takes `ctx` (threaded to the handler).
...
## Finish pass (v0.7.0) — closed gaps

A completeness review (artifact) drove a punch-list of fixes; the non-obvious ones:
- **Skill outcome counters were inert** (permanent 0/0). `SkillStore.recordOutcome(name, ok)` bumps success/fail WITHOUT re-embedding or version-bumping (content didn't change). `outcomeHook` (skills/hooks) records the outcome for every skill a turn loaded via `skill_load`, wired in `onChatResponse`. The retriever now ranks on real feedback, not just embedding similarity.
- **Budget meter leaked on failure.** Usage `record()` moved from `onChatResponse` to `beforeTurn` (right after `canSpend`), so a turn that crashes before the response still counts — the meter is the backstop for exactly those loops. `dailyCallLimit` is now a real setting (10–100k, Settings input); `UsageMeter` takes a live limit GETTER (`() => state.settings.dailyCallLimit`) so the ceiling changes without rebuilding the cached meter.
- **`authorizeTurn` is a real gate, not `true`.** Grants full permissions on owner channels (`activeChannel.kind` web/messenger/voice) + channel-less turns (email, owner-allowlisted); an unrecognized channel gets `{allowed:true, grantedPermissions:[]}` (read-only). Effective grant unchanged today; narrows for future non-owner surfaces.
- **`send_message` idempotency:** unkeyed sends key on `contentHash(title, body)` (FNV-1a, sync) instead of a random UUID, so a stall-retry dedupes. Receipt key matches.
- **Paused executions:** Think ALREADY provides `approveExecution`/`rejectExecution`/`pendingApprovals` (client-callable, they replace the paused transcript part + auto-continue) — DON'T reimplement (name-collides, TS2612 on `rejectExecution`). LabPanel calls the built-ins; we add only `rollbackExecution` (not a built-in) + `listPendingActions`.
- **Browser recording:** `browserRecording(sessionId)` was unreachable — LabPanel now has a session-recording control that downloads the rrweb JSON (replayable in any rrweb-player) with a duration/tab summary. No heavy in-app player dep.
- **Manual skills CRUD:** `saveSkill`/`deleteSkillByName` callables (reuse store create/patch/delete + embedding purge, receipted); SkillsPanel gained New/edit/delete on approved skills (was approve/reject only). `unsubscribePush` now reachable via a `disablePush` helper + on/off banner keyed on `state.pushSubscriptions`.
- **`state.revision` is live** (was dead): `bumpRevision()` fires on receipt/health data change; Receipts/Health/Lab refetch via a `useEffect(..., [revision])` (biome-ignored — revision is the trigger, load is stable).
- **Cron:** `shiftCronToUtc` now shifts each hour in a comma list ("9,17") and REFUSES step/range hours (`null` → createReminder gives a clear parse error instead of silently firing an hour off).
- **Observability:** empty/unconfigured email replies and history-search failures now log System Health events (were silent); history search THROWS on error so the UI shows a real error, not a false "no matches". CSRF `assertSameOrigin` added to `/setup/telegram-webhook`. FOUC fixed with a pre-hydration theme script in `index.html`. Mobile sidebar is an overlay drawer (was `hidden md:block` the toggle couldn't override).

## Testing

Vitest runs **two projects** (`vitest.config.ts`): a `node` project for pure-logic `*.test.ts` and a `components` project for `*.test.tsx` in `jsdom` with Testing Library (`src/test/setup.ts` = jest-dom matchers + auto-cleanup). The node project covers logic extracted out of the DO-bound `server.ts` so it unit-tests without a Worker runtime: `auth`, `lib/time` (DST/cron math), `lib/bearer` (MCP gate), `core/csrf`, `core/circuit-breaker`, `core/tool-policy`, `receipts` (keyset pagination + export), `usage` (incl. the live-limit getter), `events` (System Health log), and `skills/store` (`recordOutcome`). SQLite-backed stores (`ReceiptStore`, `EventLog`, `UsageMeter`) are exercised through a small in-memory emulator of the `this.sql` tagged template — each test builds a `fakeSql()` matching the query shapes; the skill store test fakes R2/Vectorize/AI. The components project covers `ToolApproval` (the approval gate — renders tool name/input, fires onApprove/onReject). CI (`.github/workflows/ci.yml`) runs typecheck + test + build on every push/PR; `deploy.yml` runs the test step before deploying. ⚠️ `@testing-library/dom` must be installed explicitly (peer dep not pulled by `--legacy-peer-deps`).
...
- `MCP_SERVER_URL` - MCP server URL for tool integration (optional, CLIENT side)
- `MCP_ACCESS_KEY` - Bearer token gating Holston's OWN MCP server at `/mcp` (optional secret; endpoint is 404 until set)
