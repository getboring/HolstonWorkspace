...
src/components/        ChatView, TasksPanel, McpPanel, SkillsPanel, SettingsPanel, ReceiptsPanel,
                       MemoryCard, ToolApproval, SessionList (Kumo)
...
- **Code execution + browser (Cloudflare-native)**: `getTools()` adds `createExecuteTool(this)` (Codemode — runs model code in an isolated Worker via the `LOADER` binding; exposes `state.*` = DO workspace, `tools.*`, `cdp.*` = browser) and `createBrowserTools({browser, loader})` when `BROWSER` is bound. `CodemodeRuntime` is exported from the Worker entry so the loader can instantiate it. Governed by the same `approvalMode`/`beforeToolCall` gate. `getSystemPrompt()` advertises a capability only when its binding is present (no claiming tools it lacks).
...
- **Observability**: `src/observability.ts` `subscribe()`s to the `schedule`/`chat`/`fiber`/`mcp` diagnostics channels and logs failures (scheduled-task errors, recovery exhaustion, stalled streams, MCP connect failures — the CF `observability` flag only captures console + platform metrics, not these). Wired idempotently in `onStart`. Note: MCP failure detail is in `event.payload.error`, not the event type.
